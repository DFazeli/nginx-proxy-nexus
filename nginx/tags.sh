...
for IMAGE in $(docker images | sed -e '1d' | awk '{ print $1 ":" $2 }' | grep -v 'sonatype/nexus3' | grep -v kubespray | grep -v 'nginx:1.21.6-alpine' | egrep -v 'private.repo.co|192.168.254.10|none|registry|redis|nginx-nexusproxy|alpine|postgres|centos')
do
    echo "==========================================================="
